...
@app.route('/')
def index():
    return render_template('index.html', message='AI Proctoring System')
...
def register():
    if request.method == 'POST':
        ...
        if student_id:
            success = register_face(student_id, model="cnn")
            if success:
            ...
            return redirect(url_for('result', message='Please enter a student ID.'))
...
def verify():
    if request.method == 'POST':
        student_id = request.form['student_id']
        if student_id:
            success = verify_face(student_id, model="cnn")
            if success:
                return redirect(url_for('result', message=f'Verification successful for {student_id}!'))
            else:
                return redirect(url_for('result', message=f'Verification failed for {student_id}.'))
        else:
            return redirect(url_for('result', message='Please enter a student ID.'))
